...
/// JNI implementation of [`send_notification`].
///
/// Separated to keep `send_notification` within the line limit.
#[cfg(target_os = "android")]
#[allow(clippy::too_many_lines)] // JNI boilerplate is inherently verbose
fn try_send_notification(title: &str, body: &str, tag: &str) -> Result<(), String> {
    use jni::{objects::JObject, JavaVM};
    use ndk_context::android_context;
...
pub fn send_notification(title: &str, body: &str, tag: &str) {
    match try_send_notification(title, body, tag) {
